 ...
 */
package org.openmrs.module.prep.fragment.controller.system;
...
public class ProgramsContentFragmentController {
	...
	public void controller(FragmentModel model, @SpringBean ProgramManager programManager) {
		...
		for (ProgramDescriptor descriptor : programManager.getAllProgramDescriptors()) {
			...
			programs.add(SimpleObject.create("name", program.getName(), "enrollmentForm", descriptor
			        .getDefaultEnrollmentForm().getTarget().getName(), "visitForms", visitForms, "completionForm",
			    descriptor.getDefaultCompletionForm().getTarget().getName()));
		}
