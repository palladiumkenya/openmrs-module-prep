		...
		for (Integer ptId : cohort) {
			...
			if (lastFollowUp != null && previousFollowupEncounter != null) {
				diffInMonthsBtwnFollowUpEnc = (lastFollowUp.getEncounterDatetime().getMonth() - previousFollowupEncounter
				        ...
				        + ((lastFollowUp.getEncounterDatetime().getYear() - previousFollowupEncounter.getEncounterDatetime()
				                .getYear()) * 12);
			}
			...
			List<Encounter> numOfFolloupEncounters = encounterService.getEncounters(
			...
			List<Encounter> monthlyRefillEncounters = encounterService.getEncounters(
			    Context.getPatientService().getPatient(ptId),
			    null,
			    null,
			    null,
			    null,
			    Arrays.asList(Context.getEncounterService().getEncounterTypeByUuid(
			        PrepMetadata._EncounterType.PREP_MONTHLY_REFILL)), null, null, null, false);
			...
			if (lastMonthlyRefillEncounter != null && lastFollowUp != null) {
				diffInMonthBtwnLastFollowupEncAndLastRefil = (lastMonthlyRefillEncounter.getEncounterDatetime().getMonth() - lastFollowUp
				        .getEncounterDatetime().getMonth())
